...
class ReporteController extends Controller
{
    ...
    public function reportePresentismo($fechaInicio,$fechaFinal){


        $licencias = DB::table('licencias')->whereBetween('finicio',[$fechaInicio,$fechaFinal])->join('articulos','licencias.articulo_id','articulos.id')->join('obligaciones','licencias.obligacione_id','obligaciones.id')->join('cargos','obligaciones.cargo_id','cargos.id')->join('docentes','licencias.docente_id','docentes.id')->where('articulos.descuentoPresentismo','SI')->select('licencias.*','licencias.id as id', 'articulos.articulo','articulos.descuentoPresentismo','cargos.cargo','obligaciones.numeroControl','docentes.apellidos','docentes.nombres')->get();
       ...
       // Establece el idioma a español
        ...
        $pdf= PDF::loadView('reportes.planillaPresentismo',compact('licencias','pages','mes','año'))->setPaper('A4', 'portrait');
